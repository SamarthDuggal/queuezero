...
export function HostDesk({ code }: { code: string }) {
  ...
  async function closeQueue() {
    const confirmed = window.confirm(
      "Are you sure you want to close this queue? This will permanently delete the queue and all its tickets.",
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() =>
              void copy("join", `/join/${queue.code}`)
            }
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <Stat
          ...
          value={
            queue.nowServing
              ? ticketLabel(queue.nowServing.number)
              : "—"
          }
        />
        ...
        <Stat
          label="Served"
          value={String(queue.servedToday)}
        />
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button
          disabled={closing}
          onClick={() => void closeQueue()}
          className="rounded-full border border-red-500 px-6 py-3 text-red-400 disabled:opacity-50"
        >
          {closing ? "Closing..." : "Close queue"}
        </button>

        {queue.paused ? (
          <span className="self-center text-sm text-amber">
            Joins are paused.
          </span>
        ) : null}
